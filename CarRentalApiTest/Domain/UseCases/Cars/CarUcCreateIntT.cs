...
public sealed class CarUcCreateIt : TestBase, IAsyncLifetime {
   ...
   public async Task InitializeAsync() {
      ...
      _repo.Add(_seed.Car1);
      _repo.Add(_seed.Car2);
      _repo.Add(_seed.Car3);
      await _uow.SaveAllChangesAsync("Seed cars", CancellationToken.None);
   ...
   public async Task ExecuteAsync_WithValidData_ShouldPersistCar() {
      ...
      var fromDb = await _repo.FindByIdAsync(result.Value.Id, CancellationToken.None);
      Assert.NotNull(fromDb);
      Assert.Equal("MID-001", fromDb!.LicensePlate);
      Assert.Equal("BMW", fromDb.Manufacturer);
      Assert.Equal("3 Series", fromDb.Model);
      Assert.Equal(CarCategory.Midsize, fromDb.CarCategory);
      Assert.Equal(CarStatus.Available, fromDb.CarStatus);
   }
   ...
   public async Task ExecuteAsync_MultipleCars_ShouldPersistAll() {
      ...
      var allCars = await _repo.SelectAsync(null, null, CancellationToken.None);
      Assert.True(allCars.Count >= 5); // 3 Seed + 2 new

      var compactCars = await _repo.SelectAsync(CarCategory.Compact, null, CancellationToken.None);
      Assert.True(compactCars.Count >= 1); // At least MULTI-101

      var availableCars = await _repo.SelectAsync(null, CarStatus.Available, CancellationToken.None);
      Assert.True(availableCars.Count >= 5); // All created cars should be available

      var compactAvailable = await _repo.SelectAsync(
         CarCategory.Compact,
         CarStatus.Available,
   ...
   public async Task ExecuteAsync_WithAllValidCategories_ShouldSucceed(CarCategory category) {
      ...
      var fromDb = await _repo.FindByIdAsync(result.Value.Id, CancellationToken.None);
      Assert.NotNull(fromDb);
      Assert.Equal(category, fromDb!.CarCategory);

      // Verify the car appears in category-filtered queries
      var categoryCars = await _repo.SelectAsync(category, null, CancellationToken.None);
